...
class Node
  ...
  def attack
    ...
    if target
      damage = occupant.race == 'E' ? $elf_damage : 3
      target.occupant.hp -= damage
      if target.occupant.hp < 1
        elf = target.occupant.race == 'E'
        target.occupant = nil
        return elf
      end
    end
  ...
  loop do
    while combatants.reject(&:turn_taken).length > 0
      ...
      if combatants.map(&:race).uniq.length == 1
        round -= 1 if combatants.reject(&:turn_taken).length > 0
        # pp $elf_damage
        # pp "Round: #{round}"
        # pp combatants.map(&:hp)
